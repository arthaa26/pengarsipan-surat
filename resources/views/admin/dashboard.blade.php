...
<head>
    ...
    <style>
        ...
        /* Style khusus untuk menu yang aktif (Contoh: Manajemen User) */
...
<div class="app-layout">
    <div class="sidebar">
        ...
        <div class="sidebar-menu">
            ...
            {{-- PERUBAHAN UTAMA: KIRIM SURAT diganti MANAJEMEN USER --}}
    ...
    <div class="main-content-col">
        <div class="d-flex justify-content-between align-items-center mt-3 mb-4">
            {{-- PERUBAHAN UTAMA: DASHBOARD USER diganti DASHBOARD ADMIN --}}
            ...
            <div class="dropdown">
                <div class="user-info dropdown-toggle" id="profileDropdown" data-bs-toggle="dropdown" aria-expanded="false">
                    ...
                    @else
                        <span class="user-name d-none d-sm-block">Admin User</span>
                        <div class="profile-img"><i class="bi bi-person-circle text-primary"></i></div>
                    @endauth
        ...
        {{-- CARD COUNT: Menampilkan data dari Controller. Hanya satu card seperti di gambar. --}}
        {{-- PERBAIKAN: Menambahkan class 'justify-content-center' pada row agar card berada di tengah --}}
        <div class="row g-4 mt-2 justify-content-center">
            <div class="col-12 col-sm-6 col-lg-4">
                {{-- CARD INI MIRIP DENGAN "JUMLAH SURAT" di gambar --}}
                <div class="card-box card-green justify-content-center text-center py-4 px-5">
                    <div>
                        ...
                        {{-- Data jumlah surat (total, karena di gambar hanya ada 1 card) --}}
                ...
                <table class="table table-striped table-hover mt-0">
                    <thead>
                        <tr>
                            ...
                            <th style="width: 15%;">Id_Surat</th> {{-- Diganti ke Id_Surat (sesuai gambar) --}}
                            <th style="width: 15%;">Kode Surat</th>
                            <th style="width: 30%;">Title</th>
                            <th style="width: 25%;">Isi</th>
                            <th style="width: 10%; text-align: center;">Tindakan</th> {{-- Diganti ke Tindakan (sesuai gambar) --}}
                        </tr>
                    </thead>
                    <tbody>
                        {{-- LOOPING DATA HISTORY SURAT dari Controller. Menggunakan $suratMasuk sebagai contoh data --}}
                        @forelse ($suratMasuk ?? [] as $index => $surat)
                            <tr style="color: black;">
                                <td>{{ $index + 1 }}</td>
                                <td>{{ $surat->id ?? '...' }}</td> {{-- Mengambil ID surat --}}
                                <td>{{ $surat->kode_surat ?? '...' }}</td>
                                <td>{{ $surat->title ?? '...' }}</td>
                                {{-- Memotong isi surat agar tabel rapi --}}
                                <td>{{ Illuminate\Support\Str::limit($surat->isi, 30) ?? '...' }}</td>
                                
                                {{-- KOLOM TINDAKAN (SESUAI GAMBAR: MATA DAN SAMPAH) --}}
                        ...
                        @empty
                            <tr style="color: black;">
                                <td colspan="6" class="text-center">Tidak ada data history surat yang ditemukan.</td> 
                            </tr>
                        @endforelse
                    </tbody>
                </table>
