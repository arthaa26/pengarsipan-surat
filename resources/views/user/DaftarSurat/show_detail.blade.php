...
<style>
    ...
    .action-link {
        transition: all 0.2s;
        cursor: pointer;
        text-decoration: none; /* Tambahkan agar tidak bergaris bawah default */
    }
    .action-link:hover {
        opacity: 0.8;
    }
...
{{-- BLADE LOGIC: Menghitung variabel yang diperlukan di view jika controller belum menyediakannya --}}
@php
    // Memuat relasi jika belum dimuat (praktik terbaik)
    $surat->loadMissing(['user1.faculty', 'user2', 'tujuanFaculty']);
    
    // Data Pengirim
    $pengirim = $surat->user1->name ?? 'Pengguna Tidak Dikenal';
    $fakultasPengirim = $surat->user1->faculty->name ?? 'Pusat/Non-Fakultas';
    
    // Data Tujuan
    if ($surat->user_id_2) {
        $tujuanDisplay = $surat->user2->name ?? 'Personal (Target Tidak Ditemukan)';
    } else {
        $tujuanTeks = ucwords(str_replace('_', ' ', $surat->tujuan ?? 'N/A'));
        $tujuanFakultas = $surat->tujuanFaculty->name ?? 'Seluruh Fakultas';
        
        $tujuanDisplay = "Semua " . $tujuanTeks;
        if ($surat->tujuan_faculty_id) {
            $tujuanDisplay = $tujuanTeks . " Fakultas " . $tujuanFakultas;
        }
    }
...
<div class="d-flex justify-content-between align-items-center header-detail">
    ...
    {{-- PERBAIKAN: Mengarah ke daftar surat masuk user --}}
    <a href="{{ route('user.daftar_surat.masuk') ?? '#' }}" class="btn btn-sm btn-outline-secondary">
    ...
    <div class="row">
        ...
        <div class="col-md-6 mb-3">
            ...
            {{-- Menggunakan variabel yang dihitung di atas --}}
        ...
        <div class="col-12 mt-3 pt-3 border-top">
            ...
            @if ($surat->file_path)
                <div class="d-flex gap-3 align-items-center">
                    {{-- PERBAIKAN ROUTE: Menggunakan route user/public yang benar --}}
                    <a href="{{ route('surat.view_file', $surat->id) ?? '#' }}" 
        ...
        {{-- Tombol Aksi Balas (Hanya muncul jika ini adalah surat masuk) --}}
        @if(Auth::check() && $surat->user_id_2 == Auth::id())
        <div class="col-12 mt-4 text-end">
            <a href="{{ route('DaftarSurat.reply', $surat->id) ?? '#' }}" class="btn btn-primary">
                <i class="bi bi-reply-fill me-1"></i> Balas Surat
            </a>
        </div>
        @endif
    </div>
